...
const markAvailable = (backend, select = true) => {
...
/**
Walks the candidates in order. The first backend that answers becomes the
selected provider (`state.backend`); if it reports "no window" the remaining
backends are still consulted, so an XWayland window is found through `xprop`
even when the Wayland provider sees nothing focused.
*/
export async function runFirst(method, session, backends = candidateBackends(session)) {
	let answered = false;
	for (const backend of backends) {
		...
		try {
			...
			markAvailable(backend, !answered);
			answered = true;
			if (result !== undefined) {
				return result;
			}
...
export const selectedBackend = () => state.backend;

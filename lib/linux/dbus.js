...
export function requireInterface(object, interfaceName, {name, path} = {}) {
	let iface;
	try {
		// Dbus-next throws a plain Error ("interface not found in proxy object")
		// for a missing interface; older versions returned undefined.
...
export function isServiceGone(error) {
	...
	return GONE_ERRORS.has(type) || /dbus-next is not available|DBUS_SESSION_BUS_ADDRESS|ENOENT|ECONNREFUSED|interface not found in proxy object/.test(String(error?.message));
}

...
import {candidateBackends, runFirst, selectedBackend} from '../linux.js';
import {isServiceGone, requireInterface} from './dbus.js';
import {detectDesktop, detectSession} from './session.js';
...
test('candidateBackends: Wayland sessions end with the X11 fallback', t => {
	...
	t.deepEqual(gnome, ['gnome-trackabi', 'gnome-focused-window-dbus', 'gnome-window-calls-extended', 'gnome-window-calls', 'x11']);
...
test('requireInterface: a missing interface is reported as a gone service', t => {
	const object = {getInterface: () => undefined};
	const error = t.throws(() => requireInterface(object, 'io.trackabi.ActiveWindow', {name: 'org.gnome.Shell', path: '/io/trackabi/ActiveWindow'}));
	t.is(error.type, 'org.freedesktop.DBus.Error.UnknownInterface');
	t.true(isServiceGone(error));

	const iface = {Get() {}};
	t.is(requireInterface({getInterface: () => iface}, 'x'), iface);
});

test('isServiceGone: plain TypeErrors are not treated as a missing provider', t => {
	t.false(isServiceGone(new TypeError('Cannot read properties of undefined (reading \'Get\')')));
	t.true(isServiceGone({type: 'org.freedesktop.DBus.Error.UnknownObject', message: 'x'}));
});

test.serial('runFirst: keeps walking on "no window" but selects the first provider that answered', async t => {
	const gnome = {name: 'fake-gnome', activeWindow: async () => undefined};
	const x11 = {name: 'fake-x11', activeWindow: async () => ({title: 'XWayland app'})};
	const result = await runFirst('activeWindow', undefined, [gnome, x11]);
	t.is(result.title, 'XWayland app');
	t.is(selectedBackend().name, 'fake-gnome');
});

test.serial('runFirst: a throwing provider is skipped and the next one is selected', async t => {
	const broken = {
		name: 'fake-broken', async activeWindow() {
			throw new Error('nope');
		},
	};
	const x11 = {name: 'fake-x11-2', activeWindow: async () => ({title: 'w'})};
	const result = await runFirst('activeWindow', undefined, [broken, x11]);
	t.is(result.title, 'w');
	t.is(selectedBackend().name, 'fake-x11-2');
	// The broken one stays skipped for a while.
	t.is(await runFirst('activeWindow', undefined, [broken, {name: 'fake-none', activeWindow: async () => undefined}]), undefined);
});

test('detectSession: sees through Electron\'s XDG_CURRENT_DESKTOP=Unity override', t => {
	const electronGnome = {
		XDG_SESSION_TYPE: 'wayland',
	...
	t.true(detectSession(electronGnome).isGnome);
	t.false(detectSession(electronGnome).isKde);

	const electronKde = {XDG_SESSION_TYPE: 'wayland', XDG_CURRENT_DESKTOP: 'Unity', KDE_FULL_SESSION: 'true'};
	t.true(detectSession(electronKde).isKde);
	t.false(detectSession(electronKde).isGnome);

	t.is(detectDesktop({XDG_CURRENT_DESKTOP: 'Unity', GNOME_SHELL_SESSION_MODE: 'ubuntu'}), 'Unity:GNOME');
});

test('requireInterface: dbus-next throwing for a missing interface is also a gone service', t => {
	const object = {
		getInterface() {
			throw new Error('interface not found in proxy object: io.trackabi.ActiveWindow');
		},
	};
	const error = t.throws(() => requireInterface(object, 'io.trackabi.ActiveWindow', {name: 'org.gnome.Shell', path: '/io/trackabi/ActiveWindow'}));
	t.is(error.type, 'org.freedesktop.DBus.Error.UnknownInterface');
	t.true(isServiceGone(error));
	t.true(isServiceGone(new Error('interface not found in proxy object: x')));
});
